...
"""
    ...
    for event_idx, event in enumerate(timeline_events):
        ...
        highlight_creation += f"""        highlight_{event_idx} = Rectangle(
            width=12,
...
"""
    ...
    for event_idx, event in enumerate(timeline_events):
        ...
        highlight_positioning += f"{indent}highlight_{event_idx}.stretch_to_fit_width(full_code.width + 0.2)\n"
...
"""
        ...
        remaining_time = highlight_duration - 1.0  # Subtract animation time (0.4 + 0.3 + 0.3)
        print(f"  Remaining time after animations: {remaining_time:.2f}s (duration {highlight_duration:.2f}s - 1.0s animation)")
        ...
        # Generate animation with conditional scrolling (check scroll_distance in Manim code)
        # Animation time: 0.3s (scroll if needed) + 0.4s (fade in) + 0.3s (pulse down) + 0.3s (pulse up) = 1.0s or 1.3s
        animation_timeline += f"""            # Scroll only if needed (scroll_distance > 0)
            ...
            # Highlight animation (always happens)
            self.play(highlight_{event_idx}.animate.set_opacity(0.35).set_stroke_opacity(0.8), run_time=0.4)
            self.play(highlight_{event_idx}.animate.set_opacity(0.25).set_stroke_opacity(0.6), run_time=0.3)
            self.play(highlight_{event_idx}.animate.set_opacity(0.35).set_stroke_opacity(0.8), run_time=0.3)
            remaining_time = {highlight_duration:.2f} - scroll_time - 1.0
            if remaining_time > 0:
    ...
    try:
        ...
        for block in code_blocks:
            ...
            if block['type'] == 'class':
                ...
                # Check if this loop contains other blocks (nested loops/ifs)
                ...
                # Keep if: small (3 lines) OR doesn't contain nested blocks
                # This allows highlighting outer loops that don't have nested content
