...
"""
    ...
    # Reveal ALL remaining code (anything still dim after last highlight)
    animation_timeline += f"""            # Reveal any remaining dim code
            self.play(code_lines.animate.set_opacity(1.0), run_time=0.5)
"""
    current_time += 0.7  # 0.2 for highlight fade + 0.5 for reveal
...
- **LABELS**:
  - ⚠️ CRITICAL: Use `.next_to(object, direction)` for labels, NOT `.move_to(object)`!
  - Example: `label.next_to(circle, UP)` ✅ NOT `label.move_to(circle)` ❌
  - Use different directions (UP, DOWN, LEFT, RIGHT) for different labels to avoid overlap.
...
Generate code:"""
    ...
    for attempt in range(max_attempts):
        ...
        # Check 5: Detect potential text overlap
        # Count how many Text objects
        ...
        if text_count >= 3:
            ...
            # CRITICAL: Also check for .move_to() - very common cause of overlap!
            # If many Text objects and many .move_to() calls, they're likely overlapping
            move_to_count = ai_code.count('.move_to(')
            if move_to_count >= 3:
                issues.append("Potential text overlap: multiple labels using .move_to() at same position")
        ...
        # If creating many VGroup items and stacking them UP, likely off-screen
        if vgroup_count >= 5 and next_to_up_count >= 4:
            issues.append("Potential off-screen: too many items stacked vertically")
