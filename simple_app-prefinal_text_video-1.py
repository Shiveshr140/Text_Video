...
1. Title
2. 1-2 TEXT slides (definition, explanation)
3. 1-2 ANIMATION concepts (visual representations)
...
{{
    ...
    "animation_slides": [
        {{
            "concept": "Visual concept (e.g., '3 circles showing AI capabilities')",
            "narration": "What to explain"
        }}
    ]
}}"""
        ...
        structure = json.loads(response.choices[0].message.content)
        print(f"✅ Structure: {len(structure.get('text_slides', []))} text + {len(structure.get('animation_slides', []))} animations\n")
...
"""
    ...
    # TEXT SLIDES with proper wrapping
    for idx, slide in enumerate(text_slides):
        content = slide.get('content', '')
        wait_time = max(durations[dur_idx][1] - 2.5, 2.0) if dur_idx < len(durations) else 3.0
        dur_idx += 1
        
        # Wrap text into multiple lines (max 60 chars per line)
        wrapped_lines = textwrap.wrap(content, width=60)
        
        code += f"""        # Text Slide {idx + 1} - Multi-line wrapped text
        text_lines_{idx} = VGroup()
"""
        
        for line_idx, line in enumerate(wrapped_lines):
            escaped_line = line.replace('"', '\\"')
            code += f"""        line_{idx}_{line_idx} = Text("{escaped_line}", font_size=24, font="JetBrains Mono", color=WHITE)
        text_lines_{idx}.add(line_{idx}_{line_idx})
"""
        
        code += f"""        text_lines_{idx}.arrange(DOWN, aligned_edge=LEFT, buff=0.3)
        text_lines_{idx}.move_to(ORIGIN)
        self.play(FadeIn(text_lines_{idx}, shift=UP*0.2), run_time=1.0)
        self.wait({wait_time})
        self.play(FadeOut(text_lines_{idx}), run_time=0.6)
        self.wait(0.5)
...
def generate_centered_animation(concept, narration, target_duration, client):
    """Generate CENTERED animation that MATCHES the narration"""
    
    prompt = f"""Generate Manim code to visualize: {concept}

NARRATION (AUDIO ONLY - will be spoken, NOT displayed): "{narration}"

Your animation MUST match this narration! Create visuals that illustrate what's being explained.
DO NOT create Text objects with the full narration - only create visual elements with SHORT labels.

Target duration: {target_duration:.1f} seconds

CRITICAL RULES (MUST FOLLOW):
1. **Animation MUST match the narration content** - visualize what's being said
2. **DO NOT display the narration text** - it's audio only! Only use SHORT labels (1-3 words)
3. **NO SVG files or external resources** - Use ONLY basic Manim shapes: Circle, Rectangle, Text, Arrow, Line
4. ALL elements must be CENTERED using .move_to(ORIGIN) or .shift()
5. Use .next_to() with buff=0.5 minimum to prevent overlap
6. Arrange elements horizontally: .arrange(RIGHT, buff=1.5)
7. Use font_size=22-24 for labels (not too big)
8. NO overlapping text - position labels ABOVE or BELOW elements
9. Return ONLY animation code (no class/def, FLUSH-LEFT)
10. Duration EXACTLY {target_duration:.1f}s

GOOD EXAMPLE (centered, no overlap, NO SVG):
```
# Create 3 circles horizontally
circles = VGroup()
circle1 = Circle(radius=0.7, color=BLUE)
label1 = Text("Learning", font_size=22, color=BLUE).next_to(circle1, DOWN, buff=0.3)
circles.add(VGroup(circle1, label1))

circle2 = Circle(radius=0.7, color=GREEN)
label2 = Text("Reasoning", font_size=22, color=GREEN).next_to(circle2, DOWN, buff=0.3)
circles.add(VGroup(circle2, label2))

circle3 = Circle(radius=0.7, color=YELLOW)
label3 = Text("Problem-Solving", font_size=22, color=YELLOW).next_to(circle3, DOWN, buff=0.3)
circles.add(VGroup(circle3, label3))

circles.arrange(RIGHT, buff=1.5)
circles.move_to(ORIGIN)  # CENTER IT!

self.play(LaggedStart(*[FadeIn(c, shift=UP*0.2) for c in circles], lag_ratio=0.3), run_time=2.0)
self.wait(X.X)  # Fill to {target_duration:.1f}s
```

Return ONLY animation code:"""

    response = client.chat.completions.create(
        model="gpt-4o",
        messages=[
            {"role": "system", "content": "You are a Manim expert. Follow centering rules strictly."},
            {"role": "user", "content": prompt}
        ],
        temperature=0.5
    )
